            ...
            if self.run_eval:
                ...
                for i in range(len(self.eval_loader_list)):
                    ...
                    for step_idx, data in enumerate(eval_loader):
                        ...
                        ref_mask = ref_mask.squeeze(1).permute(1, 2, 0)
                        new_ref_mask = vision.Resize(size=[896, 896], interpolation=Inter.NEAREST)(ref_mask.numpy()).transpose(2, 0, 1)
                        new_ref_mask = ms.Tensor(new_ref_mask, ms.float32)
                        logit_mask = logit_mask.squeeze(1)
                        seg_iou, seg_ap= mask_iou(new_ref_mask, logit_mask.sigmoid()>args.mask_threshold)
                ...
                for split, correct_det, correct_seg, total in split_correct_total:
                    print(f'precision/{split}_det', correct_det/total, epoch)
                    print(f'precision/{split}_seg', correct_seg/total, epoch)
...
def train_net():
    ...
    optimizer = MyAdamWeightDecayV2(params=param_groups, learning_rate=0.01 * args.lr_base,
